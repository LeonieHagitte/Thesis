...
```{r, echo=FALSE}
imputedData99 <- mice(df_dl99, m=5, method='pmm', maxit=50)
completedData99 <- complete(imputedData99, 1)
```
...
imputed_test_data4 <- mice(df_dl99, m=5, method='pmm', maxit=50)
imputed_data_all_c4 <- complete(imputed_test_data4, 1)
```
...
obj_default1 <- function(chisq, df, cfi.robust, pvalue, rmsea.robust, srmr, crel, nu) {
                     1/(1 + exp(-10 * (cfi.robust - 0.6))) + 0.5 * (1 - (1/(1 + exp(-100 * 
...
obj_default4 <- function(chisq, df, pvalue, rmsea.robust, srmr, cfi.robust, crel, nu)
                        {(1-rmsea.robust)+(1-srmr)}
# 0.125*(1-rmsea.robust) + 0.875* 0.125*(1-srmr)
...
#Integration =~ F3F5 + F3F6 + F3F7 + F3F9 /F3F1
#Communication =~ F4F3 + F4F6 + F4F7 + F4F8 
...
#### metric to scalar
Cfi 0.944, delta: .003
rmsea  0.053  ,delta: .003
srmr 0.069, delta: .003
...
library(lavaan)
library(semTools)
fit_model_ord_manu_m <- cfa(cfa_model_ord_manu, ords_mi, estimator = "WLSMV", group = "split", group.equal= "loadings")
...
## function call
out <- ezCutoffs(model = cfa_model_ord_manu, n_obs = 1000, n_rep = 10, n_cores = 1)

out <- ezCutoffs(model = cfa_model_ord_manu, n_obs = c(300, 400), n_rep = 50, fit_indices = c("cfi.scaled","rmsea.scaled","srmr"), alpha_level = 0.05, estimator = "MLR", group = "group", group.equal = c("loadings", "intercepts"), n_cores = 1 )

#out<-ezCutoffs(model = cfa_model_ord, data = ords_mi, n_rep = 1000, fit_indices = c("chisq", "cfi.scaled", "tli.scaled", "rmsea.scaled", "srmr"), alpha_level = 0.05, missing_data = FALSE, bootstrapped_ci = FALSE, n_boot = 1000, boot_alpha = 0.05, boot_internal = FALSE, n_cores = 2, estimator = "WLSMV", ordered = TRUE, group = "split", group.equal = c("loadings"))
...
fit <- sem(cfa_model_ord_manu, ords_mi)
...
# Fit the SEM model
fit_cfa_model_ords <- cfa(cfa_model_ord_manu, ords, ordered = TRUE, estimator = "WLSMV")

# Get predicted values
predicted_values <- predict(fit_cfa_model_ords)

# Check for extreme values (Haywood cases)
extreme_values <- predicted_values[which(predicted_values > quantile(predicted_values, 0.95) | 
                                         predicted_values < quantile(predicted_values, 0.05))]
...
residuals_standardized <- residuals(fit_cfa_model_ords, type = "standardized")

# Convert residuals to a vector if necessary
if (is.data.frame(residuals_standardized)) {
  residuals_vector <- unlist(residuals_standardized)
} else if (is.list(residuals_standardized)) {
  residuals_vector <- do.call(c, residuals_standardized)
} else {
  residuals_vector <- residuals_standardized
}

# Extract the covariance matrix (standardized residuals)
...
# Optional: Examine residuals at different quantiles
quantile_05 <- quantile(absolute_residuals, 0.05)
extreme_residuals_low <- residuals_vector[absolute_residuals < quantile_05]

print(extreme_residuals_low)
...
## Factor Scores / Bartlett Scores
# Convert the specific subset of items to numeric
subset_ords_fa <- ords_mi[, c("F1F2", "F1F5", "F1F6", "F1F11", 
                            "F2F1", "F2F6", "F2F13", "F2F16",
                            "F3F5", "F3F6", "F3F7", "F3F9",
                            "F4F3", "F4F6", "F4F7", "F4F8",
                            "F5F1", "F5F6", "F5F8", "F5F9")]

# Convert factors to numeric
subset_ords_fa_numeric <- data.frame(lapply(subset_ords_fa, function(x) as.numeric(as.factor(x))))

# Fit the CFA model to the numeric data
fit <- cfa(cfa_model_ord_manu, data = subset_ords_fa_numeric, ordered = TRUE)

# Summary of the model
summary(fit, fit.measures = TRUE, standardized = TRUE)

# Calculate factor scores using the fitted model
factor_scores <- lavPredict(fit, method = "Bartlett")

# Add the 'split' column to the factor scores
factor_scores_combined <- data.frame(factor_scores)
factor_scores_combined$split <- ords_mi$split
...
```{r}
# Create a data frame from specified columns of df_dl
df_result <- data.frame(
  F1F4 = df_test_dl$F1F4,
  F1F6 = df_test_dl$F1F6,
  F1F10 = df_test_dl$F1F10,
  F1F11 = df_test_dl$F1F11,
  F2F1 = df_test_dl$F2F1,
  F2F6 = df_test_dl$F2F6,
  F2F13 = df_test_dl$F2F13,
  F2F16 = df_test_dl$F2F16,
  F3F5 = df_test_dl$F3F5,
  F3F6 = df_test_dl$F3F6,
  F3F7 = df_test_dl$F3F7,
  F3F8 = df_test_dl$F3F8,
  F4F3 = df_test_dl$F4F3,
  F4F6 = df_test_dl$F4F6,
  F4F7 = df_test_dl$F4F7,
  F4F8 = df_test_dl$F4F8,
  F5F6 = df_test_dl$F5F6,
  F5F8 = df_test_dl$F5F8,
  F5F9 = df_test_dl$F5F9,
  F5F16 = df_test_dl$F5F16
)
# Calculate average scores for each construct
df_result$Comprehension <- rowMeans(df_result[, c("F1F4", "F1F6", "F1F10", "F1F11")], na.rm = TRUE)
df_result$Evaluation <- rowMeans(df_result[, c("F2F1", "F2F6", "F2F13", "F2F16")], na.rm = TRUE)
df_result$Integration <- rowMeans(df_result[, c("F3F5", "F3F6", "F3F7", "F3F8")], na.rm = TRUE)
df_result$Communication <- rowMeans(df_result[, c("F4F3", "F4F6", "F4F7", "F4F8")], na.rm = TRUE)
df_result$Statistics <- rowMeans(df_result[, c("F5F6", "F5F8", "F5F9", "F5F16")], na.rm = TRUE)

# Display the updated data frame with the new scores
print(df_result)
...
```{r}
# Create a data frame from specified columns of df_dl
df_result_manu <- data.frame(
  F1F2 = df_test_dl$F1F2,
  F1F5 = df_test_dl$F1F5,
  F1F6 = df_test_dl$F1F6,
  F1F11 = df_test_dl$F1F11,
  F2F1 = df_test_dl$F2F1,
  F2F6 = df_test_dl$F2F6,
  ...
  F3F6 = df_test_dl$F3F6,
  F3F7 = df_test_dl$F3F7,
  F3F9 = df_test_dl$F3F9,
  F4F3 = df_test_dl$F4F3,
  F4F6 = df_test_dl$F4F6,
  F4F7 = df_test_dl$F4F7,
  F4F8 = df_test_dl$F4F8,
  F5F1 = df_test_dl$F5F1,
  F5F6 = df_test_dl$F5F6,
  F5F8 = df_test_dl$F5F8,
  F5F9 = df_test_dl$F5F9
)
# Calculate average scores for each construct
df_result_manu $Comprehension <- rowMeans(df_result_manu [, c("F1F2", "F1F5", "F1F6", "F1F11")], na.rm = TRUE)
df_result_manu $Evaluation <- rowMeans(df_result_manu [, c("F2F1", "F2F6", "F2F13", "F2F16")], na.rm = TRUE)
df_result_manu $Integration <- rowMeans(df_result_manu [, c("F3F5", "F3F6", "F3F7", "F3F9")], na.rm = TRUE)
df_result_manu $Communication <- rowMeans(df_result_manu [, c("F4F3", "F4F6", "F4F7", "F4F8")], na.rm = TRUE)
df_result_manu $Statistics <- rowMeans(df_result_manu [, c("F5F1", "F5F6", "F5F8", "F5F9")], na.rm = TRUE)

# Display the updated data frame with the new scores
print(df_result_manu)
...
all_means <- data.frame(
  ...
  Comprehension = df_result$Comprehension,
  Evaluation = df_result$Evaluation,
  Integration = df_result$Integration,
  Communication = df_result$Communication,
  Statistics = df_result$Statistics
)
```

```{r}
# Combine the row means into a single data frame
all_means_manu <- data.frame(
  B5con_Row_Means = B5_con$B5con_Row_Means,
  B5o_Row_Means = B5_openness$B5o_Row_Means,
  Nfc_Row_Means = NFC$Nfc_Row_Means,
  Ict_Row_Means = ICT$Ict_Row_Means,
  Swe_Row_Means = SWE$Swe_Row_Means,
  Comprehension = df_result_manu$Comprehension,
  Evaluation = df_result_manu$Evaluation,
  Integration = df_result_manu$Integration,
  Communication = df_result_manu$Communication,
  Statistics = df_result_manu$Statistics
)
```
...
```{r}
library(apaTables)
cortable<-cor(all_means_manu, use = "pairwise" ,method = "kendall")
apa.cor.table(all_means_manu, filename = "cortable.rtf")
```

```{r}
library(apaTables)
cortable<-cor(all_means, use = "pairwise" ,method = "kendall")
apa.cor.table(all_means, filename = "cortable.rtf")
```
...
old.out <- sem(cfa_model_ord_manu, data = df_sens, ordered = TRUE)

# Fit the sensitivity analysis model
my.sa <- sa.aco(
  data = df_sens,
  model = cfa_model_ord_manu,
  sens.model = sens_model_ord,
  sample.nobs = nrow(df_sens),
  k = 10,
  max.value = 2000,
  max.iter = 100,
  opt.fun = 1,
...
```{r}
library(psych)
alpha(df_result_manu)
```

```{r}
library(MBESS)
MBESS::ci.reliability(df_result_manu, type = "omega", interval.type = "perc", B = 10000, conf.level = 0.95)
...
#Comprehension
hist(joined_mgcfa_data$F1F2, ylab='Häufigkeit', xlab='Item 1', main='Histogramm')
hist(joined_mgcfa_data$F1F5, ylab='Häufigkeit', xlab='Item 2', main='Histogramm')
hist(joined_mgcfa_data$F1F6, ylab='Häufigkeit', xlab='Item 3', main='Histogramm')
hist(joined_mgcfa_data$F1F11, ylab='Häufigkeit', xlab='Item 4', main='Histogramm')
```
...
par(mfrow=c(1,4))
#Evaluation
hist(joined_mgcfa_data$F2F1, ylab='Häufigkeit', xlab='Item 5', main='Histogramm')
hist(joined_mgcfa_data$F2F6, ylab='Häufigkeit', xlab='Item 6', main='Histogramm')
hist(joined_mgcfa_data$F2F13, ylab='Häufigkeit', xlab='Item 7', main='Histogramm')
hist(joined_mgcfa_data$F2F16, ylab='Häufigkeit', xlab='Item 8', main='Histogramm')
```

```{r}
par(mfrow=c(1,4))
#Integration 
hist(joined_mgcfa_data$F3F5, ylab='Häufigkeit', xlab='Item 9', main='Histogramm')
hist(joined_mgcfa_data$F3F6, ylab='Häufigkeit', xlab='Item 10', main='Histogramm')
hist(joined_mgcfa_data$F3F7, ylab='Häufigkeit', xlab='Item 11', main='Histogramm')
hist(joined_mgcfa_data$F3F9, ylab='Häufigkeit', xlab='Item 12', main='Histogramm')
```

```{r}
par(mfrow=c(1,4))
#Communication
hist(joined_mgcfa_data$F4F3, ylab='Häufigkeit', xlab='Item 13', main='Histogramm')
hist(joined_mgcfa_data$F4F6, ylab='Häufigkeit', xlab='Item 14', main='Histogramm')
hist(joined_mgcfa_data$F4F7, ylab='Häufigkeit', xlab='Item 15', main='Histogramm')
hist(joined_mgcfa_data$F4F8, ylab='Häufigkeit', xlab='Item 16', main='Histogramm')
```

```{r}
par(mfrow=c(1,4))
#Statistics
hist(joined_mgcfa_data$F5F1, ylab='Häufigkeit', xlab='Item 17', main='Histogramm')
hist(joined_mgcfa_data$F5F6, ylab='Häufigkeit', xlab='Item 18', main='Histogramm')
hist(joined_mgcfa_data$F5F8, ylab='Häufigkeit', xlab='Item 19', main='Histogramm')
hist(joined_mgcfa_data$F5F9, ylab='Häufigkeit', xlab='Item 20', main='Histogramm')
```

```{r}
library(car)

# Create a boxplot for the columns in joined_mgcfa_data
car::Boxplot(joined_mgcfa_data, ylab='Frequency', xlab='Items', main='Boxplot', col='darkgreen')

```

```{r}
# Means
mean_F1F2 <- mean(joined_mgcfa_data$F1F2, na.rm = TRUE)
mean_F1F5 <- mean(joined_mgcfa_data$F1F5, na.rm = TRUE)
mean_F1F6 <- mean(joined_mgcfa_data$F1F6, na.rm = TRUE)
mean_F1F11 <- mean(joined_mgcfa_data$F1F11, na.rm = TRUE)
mean_F2F1 <- mean(joined_mgcfa_data$F2F1, na.rm = TRUE)
mean_F2F6 <- mean(joined_mgcfa_data$F2F6, na.rm = TRUE)
mean_F2F13 <- mean(joined_mgcfa_data$F2F13, na.rm = TRUE)
mean_F2F16 <- mean(joined_mgcfa_data$F2F16, na.rm = TRUE)
mean_F3F5 <- mean(joined_mgcfa_data$F3F5, na.rm = TRUE)
mean_F3F6 <- mean(joined_mgcfa_data$F3F6, na.rm = TRUE)
mean_F3F7 <- mean(joined_mgcfa_data$F3F7, na.rm = TRUE)
mean_F3F9 <- mean(joined_mgcfa_data$F3F9, na.rm = TRUE)
mean_F4F3 <- mean(joined_mgcfa_data$F4F3, na.rm = TRUE)
mean_F4F6 <- mean(joined_mgcfa_data$F4F6, na.rm = TRUE)
mean_F4F7 <- mean(joined_mgcfa_data$F4F7, na.rm = TRUE)
mean_F4F8 <- mean(joined_mgcfa_data$F4F8, na.rm = TRUE)
mean_F5F1 <- mean(joined_mgcfa_data$F5F1, na.rm = TRUE)
mean_F5F6 <- mean(joined_mgcfa_data$F5F6, na.rm = TRUE)
mean_F5F8 <- mean(joined_mgcfa_data$F5F8, na.rm = TRUE)
mean_F5F9 <- mean(joined_mgcfa_data$F5F9, na.rm = TRUE)

# Standard Deviations
sd_F1F2 <- sd(joined_mgcfa_data$F1F2, na.rm = TRUE)
sd_F1F5 <- sd(joined_mgcfa_data$F1F5, na.rm = TRUE)
sd_F1F6 <- sd(joined_mgcfa_data$F1F6, na.rm = TRUE)
sd_F1F11 <- sd(joined_mgcfa_data$F1F11, na.rm = TRUE)
sd_F2F1 <- sd(joined_mgcfa_data$F2F1, na.rm = TRUE)
sd_F2F6 <- sd(joined_mgcfa_data$F2F6, na.rm = TRUE)
sd_F2F13 <- sd(joined_mgcfa_data$F2F13, na.rm = TRUE)
sd_F2F16 <- sd(joined_mgcfa_data$F2F16, na.rm = TRUE)
sd_F3F5 <- sd(joined_mgcfa_data$F3F5, na.rm = TRUE)
sd_F3F6 <- sd(joined_mgcfa_data$F3F6, na.rm = TRUE)
sd_F3F7 <- sd(joined_mgcfa_data$F3F7, na.rm = TRUE)
sd_F3F9 <- sd(joined_mgcfa_data$F3F9, na.rm = TRUE)
sd_F4F3 <- sd(joined_mgcfa_data$F4F3, na.rm = TRUE)
sd_F4F6 <- sd(joined_mgcfa_data$F4F6, na.rm = TRUE)
sd_F4F7 <- sd(joined_mgcfa_data$F4F7, na.rm = TRUE)
sd_F4F8 <- sd(joined_mgcfa_data$F4F8, na.rm = TRUE)
sd_F5F1 <- sd(joined_mgcfa_data$F5F1, na.rm = TRUE)
sd_F5F6 <- sd(joined_mgcfa_data$F5F6, na.rm = TRUE)
sd_F5F8 <- sd(joined_mgcfa_data$F5F8, na.rm = TRUE)
sd_F5F9 <- sd(joined_mgcfa_data$F5F9, na.rm = TRUE)
...
# Print means
cat("Means of items:\n")
cat("Mean of F1F2: ", mean_F1F2, "\n")
cat("Mean of F1F5: ", mean_F1F5, "\n")
cat("Mean of F1F6: ", mean_F1F6, "\n")
cat("Mean of F1F11: ", mean_F1F11, "\n")
cat("Mean of F2F1: ", mean_F2F1, "\n")
cat("Mean of F2F6: ", mean_F2F6, "\n")
cat("Mean of F2F13: ", mean_F2F13, "\n")
cat("Mean of F2F16: ", mean_F2F16, "\n")
cat("Mean of F3F5: ", mean_F3F5, "\n")
cat("Mean of F3F6: ", mean_F3F6, "\n")
cat("Mean of F3F7: ", mean_F3F7, "\n")
cat("Mean of F3F9: ", mean_F3F9, "\n")
cat("Mean of F4F3: ", mean_F4F3, "\n")
cat("Mean of F4F6: ", mean_F4F6, "\n")
cat("Mean of F4F7: ", mean_F4F7, "\n")
cat("Mean of F4F8: ", mean_F4F8, "\n")
cat("Mean of F5F1: ", mean_F5F1, "\n")
cat("Mean of F5F6: ", mean_F5F6, "\n")
cat("Mean of F5F8: ", mean_F5F8, "\n")
cat("Mean of F5F9: ", mean_F5F9, "\n")

# Print standard deviations
cat("\nStandard Deviations of items:\n")
cat("SD of F1F2: ", sd_F1F2, "\n")
cat("SD of F1F5: ", sd_F1F5, "\n")
cat("SD of F1F6: ", sd_F1F6, "\n")
cat("SD of F1F11: ", sd_F1F11, "\n")
cat("SD of F2F1: ", sd_F2F1, "\n")
cat("SD of F2F6: ", sd_F2F6, "\n")
cat("SD of F2F13: ", sd_F2F13, "\n")
cat("SD of F2F16: ", sd_F2F16, "\n")
cat("SD of F3F5: ", sd_F3F5, "\n")
cat("SD of F3F6: ", sd_F3F6, "\n")
cat("SD of F3F7: ", sd_F3F7, "\n")
cat("SD of F3F9: ", sd_F3F9, "\n")
cat("SD of F4F3: ", sd_F4F3, "\n")
cat("SD of F4F6: ", sd_F4F6, "\n")
cat("SD of F4F7: ", sd_F4F7, "\n")
cat("SD of F4F8: ", sd_F4F8, "\n")
cat("SD of F5F1: ", sd_F5F1, "\n")
cat("SD of F5F6: ", sd_F5F6, "\n")
cat("SD of F5F8: ", sd_F5F8, "\n")
cat("SD of F5F9: ", sd_F5F9, "\n")
...
model_manu <- '
# Measurement model
Comprehension =~ F1F2 + F1F6 + F1F8 + F1F13 
Evaluation =~ F2F1 + F2F6 + F2F13 + F2F16 
Integration =~ F3F5 + F3F6 + F3F7 + F3F9 
Communication =~ F4F3 + F4F6 + F4F7 + F4F8 
Statistics =~ F5F8 + F5F9 + F5F13 + F5F16
...
Communication ~~ Statistics
'
fit_model_manu <- cfa(model_manu, ords, ordered = TRUE, estimator = "WLSMV")
summary(fit_model_manu, fit.measures = TRUE, standardized = TRUE)

inspect(fit_model_manu,'fit')
```