...
powerMI <- semPower.powerMI(
  ...
  loadM = list(.5, .6), # baseline loadings for two groups
  tau = list(rep(0.0, 20), # intercepts for 20 indicators in the first group
             rep(seq(.1, .5, .1), each = 4)), # intercepts for 20 indicators in the second group, with increments
  alpha = .05, 
  power = .80,
  N = list(.80, .20) # 80/20 sized groups for split
)
# Show summary
summary(powerMI)
...
```{r}
set.seed(2028) #did yield different results
# Load the necessary libraries
library(stuart)
...
fit_cfa_model_mock <- cfa(cfa_model_m, df_train, 
                       missing = "fiml", 
                       estimator = "MLR", 
                       em.h1.iter.max = 1000)

summary(fit_cfa_model_mock, fit.measures = TRUE, standardized = TRUE)

inspect(fit_cfa_model_mock, 'fit')
```
...
```{r, echo=FALSE}
set.seed(2025) # now properly seed command
