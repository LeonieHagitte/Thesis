...
```{r}
#F2F17, F2F4, F3F1 and F3F9 deleted
fs<-list(Comprehension=c("F1F1",  "F1F2",  "F1F3",  "F1F4",  "F1F5",  "F1F6",  "F1F7",  "F1F8",  "F1F9",  "F1F10", "F1F11",                              "F1F12", "F1F13", "F1F14"),
           ...
           Integration=c("F3F2",'F3F3' ,"F3F4",  "F3F5",  "F3F6",  "F3F7", "F3F8"),
...
```
#### configural to metric
Cfi scaled: .947  delta: .007
rmsea scaled: .056 delta: .001
srmr delta: .005
...
df_dl$Comprehension <- rowMeans(df_dl[, c("F1F4", "F1F6", "F1F10", "F1F11")], na.rm = TRUE)
df_dl$Evaluation <- rowMeans(df_dl[, c("F2F1", "F2F6", "F2F13", "F2F16")], na.rm = TRUE)
df_dl$Integration <- rowMeans(df_dl[, c("F3F5", "F3F6", "F3F7", "F3F8")], na.rm = TRUE)
df_dl$Communication <- rowMeans(df_dl[, c("F4F3", "F4F6", "F4F7", "F4F8")], na.rm = TRUE)
df_dl$Statistics <- rowMeans(df_dl[, c("F5F6", "F5F8", "F5F9", "F5F16")], na.rm = TRUE)
...
all_means <- data.frame(
  ...
  Comprehension = df_dl$Comprehension,
  Evaluation = df_dl$Evaluation,
  Integration = df_dl$Integration,
  Communication = df_dl$Communication,
  Statistics = df_dl$Statistics
)
```
...
```{r}
df_sens_m <- data.frame(
  F1F2 = joined_mgcfa_data$F1F3,
  F1F5 = joined_mgcfa_data$F1F6,
  F1F6 = joined_mgcfa_data$F1F12,
  F1F14 = joined_mgcfa_data$F1F14,
  F2F6 = joined_mgcfa_data$F2F5,
  F2F10 = joined_mgcfa_data$F2F12,
  F2F12 = joined_mgcfa_data$F2F14,
  F2F13 = joined_mgcfa_data$F2F16,
  F3F2 = joined_mgcfa_data$F3F3,
  F3F6 = joined_mgcfa_data$F3F4,
  F3F7 = joined_mgcfa_data$F3F7,
  F3F9 = joined_mgcfa_data$F3F8,
  F4F1 = joined_mgcfa_data$F4F1,
  F4F6 = joined_mgcfa_data$F4F2,
  F4F7 = joined_mgcfa_data$F4F3,
  F4F8 = joined_mgcfa_data$F4F8,
  F5F1 = joined_mgcfa_data$F5F3,
  F5F6 = joined_mgcfa_data$F5F6,
  F5F7 = joined_mgcfa_data$F5F15,
  F5F18 = joined_mgcfa_data$F5F18
)

# Compute the variance-covariance matrix
cov_matrix_m <- cov(df_sens_m, use = "complete.obs")
```
...
polychoric_corr <- hetcor(df_sens)

var_names <- colnames(df_sens)  # Get the variable names from your data

corr_matrix <- polychoric_corr$correlations  # Extract the correlation matrix

# Get the standard deviations of the ordinal variables
std_devs <- apply(df_sens, 2, sd)

# Convert correlation matrix to covariance matrix
cov_matrix <- diag(std_devs) %*% corr_matrix %*% diag(std_devs)
# Assign the names to the covariance matrix
colnames(cov_matrix) <- var_names
```

```{r}
# STEP 2: Set up analytic model and sensitivity anaysis model
# Original Analytic Model
cfa_model_m <- '
# Measurement model
Comprehension =~ F1F2 + F1F5 + F1F6 + F1F14 
Evaluation =~ F2F6 + F2F10 + F2F12 + F2F13 
...
Communication ~~ Statistics
  ...
  # Sensitivity analysis with phantom variable
...
old.model = cfa_model_m
old.out  = sem(model = cfa_model_m, df_sens_m, estimator = "MLR")
...
old.model = cfa_model_ord
old.out  = sem(model = cfa_model_ord, df_sens, ordered = TRUE, estimator = "WLSMV")
...
my.sa <-sa.aco(model = cfa_model_m, sens.model = sens_model_m, sample.cov = cov_matrix_m,
               sample.nobs = 616, k = 100, max.value= 2000, max.iter = 1000,
               opt.fun = 2, ## 
               paths = c(1:20), seed = 1, verbose = FALSE)
```
...
## EFA Investigation 
```{r}
efa_fit <- efa(imputed_training_data_c[1:69], rotation = "varimax", nfactors = 1:10)
summary(efa_fit)
```
################################################
```{r}
...
# Define the selected items
selected_items1 <- c("F3F2")

# Modify the imputed_training_data_c to merge answers 0 and 1 into 1 for these selected items
imputed_test_data_merged4 <- imputed_test_data_merged4 %>%
  mutate(across(all_of(selected_items1), ~ ifelse(. %in% c(1, 2), 2, .)))

print(imputed_test_data_merged4)

```

```{r}
value_counts_merged4_test <- imputed_test_data_merged4 %>%
  pivot_longer(everything(), names_to = "Variable", values_to = "Value") %>%
  count(Variable, Value) %>%
  pivot_wider(names_from = Value, values_from = n, values_fill = list(n = 0))
...
# Define the selected items
selected_items2 <- c("F1F8","F2F2","F2F5")

# Modify the imputed_training_data_ic to merge answers 0 and 1 into 1 for these selected items
imputed_training_data_merged4 <- imputed_training_data_merged4 %>%
  mutate(across(all_of(selected_items2), ~ ifelse(. %in% c(0, 1), 1, .)))

print(imputed_training_data_merged4)
...
# Define the selected items
selected_items2 <- "F1F2"

# Modify the imputed_training_data_ic to merge answers 0 and 2 into 2 for these selected items
imputed_training_data_merged4 <- imputed_training_data_merged4 %>%
  mutate(across(all_of(selected_items2), ~ ifelse(. %in% c(0, 2), 2, .)))
...
# Define the selected items
selected_items2 <- "F1F5"

# Modify the imputed_training_data_ic to merge answers 0 and 2 into 2 for these selected items
imputed_training_data_merged4 <- imputed_training_data_merged4 %>%
  mutate(across(all_of(selected_items2), ~ ifelse(. %in% c(0, 1), 2, .)))
