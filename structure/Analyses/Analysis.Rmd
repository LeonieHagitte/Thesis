...
# Load necessary libraries
library(stuart)
library(lavaan)  # Assuming the model fitting uses lavaan
set.seed(2024)   # For reproducibility

# Function to simulate data
simulate_data <- function(n, num_variables) {
  # Create random normal data with the same structure as your original data
  data <- matrix(rnorm(n * num_variables), nrow = n, ncol = num_variables)
  colnames(data) <- paste0("V", 1:num_variables)
  return(as.data.frame(data))
}

# Number of simulations and variables
num_simulations <- 10
num_variables <- 20  # Change according to your actual data
sample_size <- nrow(imputed_training_data_c)  # Same sample size as your original data

# List to store results from simulations
simulated_results <- vector("list", num_simulations)

# Loop over the number of simulations
for (i in 1:num_simulations) {
  # Simulate data
  simulated_data <- simulate_data(sample_size, num_variables)
  
  # Define a model using lavaan syntax
  model <- ' # Define your model structure here
             # For example: factor1 =~ V1 + V2 + V3 + V4'
  
  # Fit the model to the simulated data
  fit <- cfa(model, data = simulated_data)
  
  # Extract fit indices
  fit_indices <- fitMeasures(fit, c("aic", "bic", "rmsea", "cfi"))
  
  # Store the results
  simulated_results[[i]] <- fit_indices
}

# Initialize vectors to store averaged fit indices
average_simulated_fit_indices <- list(
  AIC = numeric(num_simulations),
  BIC = numeric(num_simulations),
  RMSEA = numeric(num_simulations),
  CFI = numeric(num_simulations)
)

# Collect fit indices from each simulation
for (i in 1:num_simulations) {
  average_simulated_fit_indices$AIC[i] <- simulated_results[[i]]["aic"]
  average_simulated_fit_indices$BIC[i] <- simulated_results[[i]]["bic"]
  average_simulated_fit_indices$RMSEA[i] <- simulated_results[[i]]["rmsea"]
  average_simulated_fit_indices$CFI[i] <- simulated_results[[i]]["cfi"]
}

# Compute the average fit indices
average_fit_indices_simulated <- sapply(average_simulated_fit_indices, mean)

# Print the average fit indices from simulations
print("Average Fit Indices from Simulated Data:")
print(average_fit_indices_simulated)
```
