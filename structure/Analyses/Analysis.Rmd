...
value_counts_test <- count_unique_values(imputed_test_data_c)
...
```
```{r}
# Create a list of variables with different counts in Unique_categories_train and Unique_categories_test
variables_with_different_counts <- discrepancies %>%
  filter(Unique_categories_train != Unique_categories_test) %>%
  pull(Variable)

# Print list of variables with different counts
print(variables_with_different_counts)
...
print(ords_c)
```
## Preparing a data frame for the MG-CFA
```{r}
# Add the 'split' column to each data frame
...
library(lavaan)
library(semTools)
fit_cfa_model_ords_m <- cfa(cfa_model_ord, joined_mgcfa_data, estimator = "WLSMV", group = "split", group.equal= "loadings")
...
#### metric to scalar
```{r}
fit_cfa_model_ords_scalar <- cfa(cfa_model_ord, joined_mgcfa_data, estimator = "WLSMV", group = "split",
                            group.equal = c('loadings','intercepts'))
...
df_dl$Comprehension <- rowMeans(df_dl[, c("F1F3", "F1F6", "F1F12", "F1F14")], na.rm = TRUE)
df_dl$Evaluation <- rowMeans(df_dl[, c("F2F5", "F2F12", "F2F14", "F2F16")], na.rm = TRUE)
df_dl$Integration <- rowMeans(df_dl[, c("F3F3", "F3F4", "F3F7", "F3F8")], na.rm = TRUE)
df_dl$Communication <- rowMeans(df_dl[, c("F4F1", "F4F2", "F4F3", "F4F8")], na.rm = TRUE)
df_dl$Statistics <- rowMeans(df_dl[, c("F5F3", "F5F6", "F5F15", "F5F18")], na.rm = TRUE)
