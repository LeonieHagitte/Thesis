...
For this study, a set of 20 items from a set of 71 items are selected, to form a questionnaire. The data literacy self rating scale is optimated for model fit criteria and composite reliability (RMSEA, SRMR, CFI \& McDonalds \(\omega\)) as well as variability in the difficulty of items. Those criteria will be defined in the objective function (equations 1 \& 2) in `stuart'Schultze (\citeproc{ref-schultze2017}{2017}).
...
Validation with the test data is conducted (using the `crossvalidate' function in `stuart') to assess the invariance of the measurement models between the training and testing datasets. Invariance levels are measured with the `max.invariance' function. Invariance is necessary to claim that the scale validation has worked.
