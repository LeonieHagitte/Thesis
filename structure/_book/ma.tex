...
Careless or inattentive response patterns were analyzed by the ATC items. Furthermore, the data was checked for outliers. Because of the planned random missings in the data, those missings were imputed via the `mice' package, with predictive mean matching. Also for model estimation, full information maximum likelihood was used to handle the missing data.
...
The provided sample is divided into two subsets (i.e.~training data and test data) using the `holdout' function in `stuart'. The specified item-selection procedure is applied to the training data. The training data is undergoing k-fold cross validation (k=3), using the `kfold' function in `stuart'. Those kfolded selections are then again iterated three times, to enhance the stability of the solution even more. The other sample, the test data, is used for evaluation of the final models performance, as well as the latent correlations with the convergent measures.
...
Cross-validation of the MG-CFA with the test-data indicated that the assumption of scalar invariance holds across the two subsamples: \(X^{2}\)(df = 389, N = 373) = 659.746, p \textless{} 0.001, CFI = .94, SRMR = .069, RMSEA = .053; \(\Delta\text{CFI}\) = 0.003 , \(\Delta\text{SRMR}\) = .003, \(\Delta\text{RMSEA}\) = .003.

Due to the really low loadings of some of the factors onto some respective items. Those items were exchanged manually to explore the resulting impact on the overall model fit, as well as on the MI testing. The manually adapted solution exhibits good model fit (according to \citeproc{ref-hu1999}{Hu \& Bentler, 1999}) with ordered data and WLSMV as estimator: Satorra-Bentler-\(X^{2}\) (df = XXX, N = 373) = XXX, p \textless{} XXX, CFI = XXX, TLI = .XXX, SRMR = XXX, RMSEA = .XXX, 90\%-CIRMSEA {[}.XXX; .XXX{]}.
...
All factor loadings including standard errors can be found in the Supplementary Material.
Cross-validation of the MG-CFA with the test-data indicated that the assumption of scalar invariance holds across the two subsamples: \(X^{2}\)(df = XXX, N = XXX) = XXX, p \textless{} XXX, CFI = XXX, SRMR = XXX, RMSEA = XXX; \(\Delta\text{CFI}\) = .XXX, \(\Delta\text{SRMR}\) = .XXX, \(\Delta\text{RMSEA}\) = XXX.

\begin{figure}

{\centering \includegraphics[width=2\linewidth]{images/Model5} 
...
For the latent correlations, Kendall's rank correlation coefficient - Kendalls tau was estimated, because the data was not normally distributed. The factors (Comprehension, Evaluation, Integration, Communication \& Statistics) of the data literacy scale correlated moderately to highly with the SWE-IV-16( \(\tau\) =.36, \(\tau\) =.44, \(\tau\) =.36, \(\tau\) =.50, \(\tau\) =.43 ; p \textless{} .01). The factors of data literacy showed small to moderate correlations with the NFC-K(\(\tau\) =.17, p \textless{} .01; \(\tau\) =.24, p \textless{} .01; \(\tau\) =.27, p \textless{} .01; \(\tau\) =.32, p \textless{} .01; \(\tau\) =.36, p \textless{} .01). The factors of the data literacy scale showed moderate correlations with the general items of the ICT-SC25(\(\tau\) =.26, \(\tau\) =.20, \(\tau\) =.19, \(\tau\) =.41, \(\tau\) =.28 , p \textless{} .01). The factors Evaluation, Integration and Communication correlated slightly negative with the openness of the BFI-10(\(\tau\) = -.14, p \textless{} .01; \(\tau\) = -.10, p \textless{} .05; \(\tau\) = -.11, p \textless{} .05). Openness did not correlate statistically significant with the other factors. The factors of the data literacy scale correlated slightly positive with conscientiousness of the BFI-10(\(\tau\) = .14, p \textless{} .01; \(\tau\) = .13, p \textless{} .01; \(\tau\) = .17, p \textless{} .01; \(\tau\) = .16, p \textless{} .01; \(\tau\) = .18, p \textless{} .01). The latent correlations with respective confidence intervalls are also displayed in table 3.

\subsection{Reliability and Sensitivity Analysis}\label{reliability-and-sensitivity-analysis}
